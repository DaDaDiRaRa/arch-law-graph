...
- **graph.json**: node≈4708, edge≈14383 — 법령 19 + 고시 17 + 조례 22(서울 4·부산 3·인천 3·대구 3·대전 3·광주 3·울산 3) + 대법원 판례 100 + 법령해석례 150 (Phase 1–4 완료 + Stage 1–2b + 판례·해석례 확대 + 조례 7개 광역시 + HWP 폴백)
- **기준 조회(결정-등급 카드)**: 검색-퍼스트 외 `📐 기준 조회` 모드. 용도지역→건폐율·용적률·일조 / 건물용도→주차·이격 / 연면적→조경을 **국가 vs 도시 적용**으로 나란히 + 근거 조문 칩(클릭→원문 Reader) + 연결 판례·해석례 자동 수집. **멀티리전 7개 광역시 — 용도지역·조경은 7개시(서울·부산·인천·대구·대전·광주·울산), 주차·이격은 3개시(서울·부산·인천) 데이터 보유.** 데이터 없는 카드는 "데이터 준비 중" 안내(SearchView pkRegion/sbRegion guard). 인천 주차는 도시지역/관리지역 2단. 울산 건폐율·용적률은 도시계획 조례 별표24(HWP)에서 추출.
...
1. **신규 4개시 주차·이격 카드 큐레이션** — 대구·대전·광주·울산의 `PARKING_REGIONS`·`SETBACK_REGIONS` 등재. 주차 별표(대구 별표2·울산 별표6 등)·건축조례 대지안의공지 별표(대구 별표3·울산 별표2)에서 추출. ⚠ 광주 주차 별표2·일부는 HWP 변환 빈 값 → 신호 보강 필요할 수 있음.
2. **조례 지자체 추가 확장** — `ORDIN_GROUP`에 세종·경기(도 단위 구조 상이) 등. `*_REGIONS`에 적용값 등재.
