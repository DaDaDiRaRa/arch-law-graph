...
- **기준 조회(결정-등급 카드)**: 검색-퍼스트 외 `📐 기준 조회` 모드. 용도지역→건폐율·용적률·일조 / 건물용도→주차·이격 / 연면적→조경을 **국가 vs 도시 적용**으로 나란히 + 근거 조문 칩(클릭→원문 Reader) + 연결 판례·해석례 자동 수집. **멀티리전 13개 지자체 카드.** 카드별 데이터 보유:
  ...
  - 주차: 10개(서울·부산·인천·대구·울산·세종·용인·고양·창원·제주) — 대전·광주·수원 보류
  - 이격: 10개(서울·부산·인천·대구·울산·세종·수원·용인·고양·창원) — 대전·광주·제주 보류
  - 데이터 없는 카드는 "데이터 준비 중" 안내(SearchView pkRegion/sbRegion guard). 인천 주차·제주 주차는 2단(도시지역/관리지역, 가/나지역). 울산·창원 건폐율·용적률, 다수 도시 주차·이격 별표는 도시계획·건축 조례 별표(HWP)에서 추출. **경기도는 도 단위(시군 재위임)라 카드 미생성 — 조례는 graph에 두어 검색·RAG에만 활용.** 보류 사유: 대전·광주 주차 별표·광주·제주 이격 별표는 제목이 일반("별표 N")이라 신호 게이트 미통과 + 일부 표지판/색채 별표 → 직접 fetch·검증 필요.
...
1. **보류 도시 주차·이격 마무리** — 대전·광주 주차, 광주·제주 이격 별표는 제목이 일반("별표 N")이라 신호 미통과 → 직접 fetch(MST→별표번호→HWP)해 검증 후 PARKING/SETBACK_REGIONS 등재. 수원 주차는 조례 명칭 확인 후 ORDIN_GROUP 등재부터.
2. **별표 신호 게이트 개선** — 일반 제목("별표 N") 데이터표가 빈 값으로 남음. 참조 조문("별표 N과 같다")에서 데이터표 별표 번호를 역추적해 HWP 변환하는 로직 검토.
