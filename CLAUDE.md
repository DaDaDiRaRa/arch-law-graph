...
| E-11 재빌드에서 조례 73건(광주·전남 전체·제주) fetch 조용히 누락 — 개별 실패가 로그에 묻혀 회귀 테스트(랜드스케이프 extractor_sync)가 우연히 잡아냄 | 근본원인 조사 결과 **전남·광주가 "전남광주통합특별시"로, 광주광역시가 "(구)광주광역시"로 지자체기관명이 실제로 개편**(법제처 API 반영) — 재시도로는 해결 안 됨. `inventory_ordin.py` 재실행으로 `ordin_group.py` 새 기관명 갱신 후 재빌드 → 554/554 fetch 성공. `build_graph.py`에 조례 fetch 실패 요약 로그(`ordin_failed`) + 재시도(2·5초) 안전장치 추가(향후 진짜 일시적 실패용) |
| 제주 조경 카드가 가리키던 "제주특별자치도 건축 조례" 자체가 폐지("건축 기본 조례"=정책위원회 총괄 조례로 축소, 조경 조항 없음) — 근거조문(제23조) 소실 | 후속 소재 미확인 상태로 값을 유지하면 지어낸 근거가 됨 → `landscape.js`에서 제주 항목 가드 처리(값 미표시), 골든 갱신. 정확한 이관처 조례 확인은 후속 과제 |
