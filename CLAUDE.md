...
web/src/
  ...
  zoning.js parking.js setback.js landscape.js incentive.js  # 도시별(13개 지자체) 큐레이션 기준 데이터(국가 vs 도시)
...
- **graph.json**: node≈6323, edge≈19224 — 법령 19 + 고시 17 + 조례 41 + 대법원 판례 100 + 법령해석례 150 (Phase 1–4 완료 + Stage 1–2b + 판례·해석례 확대 + 조례 13개 지자체 + HWP 폴백)
  - 조례 41 = 7개 광역시(서울·부산·인천·대구·대전·광주·울산)×3 + 세종×3 + 제주×3 + 특례시(용인·고양·창원)×3 + 수원×2(주차 명칭 상이로 보류) + 경기도×2(도 단위) + 서울 도시계획 시행규칙 1.
- **기준 조회(결정-등급 카드)**: 검색-퍼스트 외 `📐 기준 조회` 모드. 용도지역→건폐율·용적률·일조 / 건물용도→주차·이격 / 연면적→조경을 **국가 vs 도시 적용**으로 나란히 + 근거 조문 칩(클릭→원문 Reader) + 연결 판례·해석례 자동 수집. **멀티리전 13개 지자체 카드.** 카드별 데이터 보유:
  ...
  - 이격: 13개(전체)
  - **완화·혜택(신규 서브탭)**: 13개 전체 — 공개공지(대상·면적·완화) + 부설주차 면제·완화(시행령 제8조). 행별 국가 vs 도시. `incentive.js`/`IncentiveCard.jsx`.
...
13. 🔄 Stage 8 — 실무 토픽(완화·혜택) 카드 (Phase 1 완료 2026-06-26). 새 `완화·혜택` 서브탭 — 공개공지(대상·면적·완화) + 부설주차 면제·완화, 13개 지자체. graph에 이미 있는 조문에서 큐레이션(빌더 변경 없음). Phase 2(용적률·건폐 완화 인센티브)·Phase 3(친환경 — `녹색건축물 조성 지원 조례` fetch 필요) 예정.
...
1. **완화·혜택 Phase 2** — 용적률·건폐 완화 인센티브(임대주택·기부채납·감염병 등) 카드. 도시계획조례 완화 조문(서울 제51·47조 등, graph에 있음) 큐레이션.
2. **완화·혜택 Phase 3 (친환경)** — 인증등급별 용적률 완화%·의무 대상. 먼저 `녹색건축물 조성 지원 조례`를 ORDIN_GROUP에 fetch(현재 창원만 graph에 있음) → 큐레이션.
3. **수원 주차 출처 조사** — 일반 주차장 설치 조례가 법제처 DB에 없음. 다른 조례에 기준 있는지 확인(유일한 보류 카드).
4. **검색 고도화** — 초성 검색, 동의어 (예: "건폐율"↔"건축면적의 비율")
5. **인용 관계 시각화** — 특정 조문 선택 시 인용/피인용 미니 그래프 패널
6. **채팅 품질 개선** — 벡터 임베딩 기반 RAG, 주소→용도지역 자동 조회 연동(VWorld 이슈 해결 후)
