...
1. **수원 주차 카드** — 유일한 미완 카드. 수원시 일반 주차장 설치 조례가 법제처 ordin DB에 없음(보훈시설 조례만). 건축 조례·시행규칙 등 다른 조례에 부설주차 기준이 있는지 조사 후 등재.
2. **심의 별도 법령 정밀화** — 심의 대상 탭의 교통·환경·경관·지하안전·재해 평가는 현재 "참고"(근거 칩 없음). 도시교통정비촉진법·경관법·지하안전관리특별법 등을 graph에 추가하면 정확한 규모 임계값 + 근거 칩 제공 가능.

### B. 데이터 확장

3. **조례 지자체 추가** — 성남·청주·전주·천안 등 인구 50만↑ 대도시. 특례시처럼 지자체기관명 "도 시" 형식 주의(현재 13개 지자체 + 녹색건축 13).
...
9. **데이터 자동갱신** — 로컬 작업 스케줄러(매일 09:00, `scripts/refresh_local.ps1`)가 graph.json 갱신. 신규 조례 54개도 자동 추적됨.

> 실무 가치 우선순위: **B-3(지자체 추가, 커버리지)** 와 **A-2(심의 별도법령, 탭 완성도)** 가 높음.
