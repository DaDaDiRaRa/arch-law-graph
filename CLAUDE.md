...
13. 🔄 Stage 8 — 실무 토픽(완화·혜택) 카드. 새 `완화·혜택` 서브탭, 13개 지자체. graph에 이미 있는 조문에서 큐레이션(빌더 변경 없음).
    ...
    - Phase 3 ✅ (2026-06-26) — 친환경(녹색건축·ZEB 인증등급별 완화·의무). **완화비율이 국토부 고시「건축물의 에너지절약설계기준」별표9로 전국 통일** → 도시 조례 fetch 불필요(이미 graph에 별표9·법 제15조·시행령 제11·12조 보유). 녹색건축 최우수 6%·우수 3% / ZEB 1~5등급 15~11% / 시범사업 10%.
...
1. **완화·혜택 후속(선택)** — 도시별 녹색건축 조례 재정·세제 지원 차이, 건축위원회 심의 대상 체크리스트 카드 등(graph에 조문 있음). 심의 대상은 새 항목/탭 후보.
