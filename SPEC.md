...
| C-1 — 카드 회귀 테스트 | pytest 9검사 + 골든 + refresh 게이트 + GitHub Actions CI |
...
| 항목 | 상태 |
|---|---|
| 구조화 조회 엔드포인트 확장 (`/api/article/{id}`, `/api/standard?city=&zone=`) | 미구현 (B그룹, `doc/B-law-api-plan.md`) |
| 카드 데이터 단일 소스화 (`data/standards.json`) | 미구현 (API·MCP 전조) |
| 멀티턴 컨텍스트 (이전 대화 주입) | 미구현 |
| 군(郡) 지역 카드 | 미구현 (검색·RAG·corpus만) |
| 판례·해석례 신규 토픽 키워드 확대 | 부분 (cap 100/150 유지) |
| 입법예고/개정 진행중 추적 | 미구현 (advanced, 현행만 보유) |
| `startup` 이벤트 → lifespan 핸들러 이전 | 미구현 (B-7) |
