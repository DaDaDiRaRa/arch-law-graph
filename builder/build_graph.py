...
ORDIN_GROUP = [
    ...
    # ── Stage 2b: 도시 확장 — 서울 템플릿(건폐율·용적률·일조·주차·이격·조경) 복제용 핵심 3종 ──
    # 정확명이 어긋나면 fetch_ordin 이 "✗ 조례 검색 결과 없음" 출력 → 재빌드 로그로 확인 후 보정.
    ("부산광역시", "부산광역시 도시계획 조례"),          # 건폐율·용적률
    ("부산광역시", "부산광역시 건축 조례"),              # 일조·이격·조경
    ("부산광역시", "부산광역시 주차장 설치 및 관리 조례"),  # 부설주차
    ("인천광역시", "인천광역시 도시계획 조례"),          # 건폐율·용적률
    ("인천광역시", "인천광역시 건축 조례"),              # 일조·이격·조경
    ("인천광역시", "인천광역시 주차장 설치 및 관리 조례"),  # 부설주차
    # ── 조례 지자체 확장: 대구·대전·광주·울산 4개 광역시(각 도시계획·건축·주차 3종) ──
    # ⚠ 대전·광주의 주차 조례는 '주차장 조례'(짧은 명칭) — 다른 도시는 '주차장 설치 및 관리 조례'.
    ("대구광역시", "대구광역시 도시계획 조례"),
    ("대구광역시", "대구광역시 건축 조례"),
    ...
    ("경기도", "경기도 도시계획 조례"),  # 도 단위 기본값(자체 조례 없는 시군 적용)
    ("경기도", "경기도 건축 조례"),
    # ── Stage 10: B-3 신규 4개 도시(성남·청주·전주·천안) — 인구 50만↑ ──
    # ⚠ 성남 주차 조례는 '설치 및 관리 운영에 관한 조례'(다른 도시와 명칭 다름).
    # ⚠ 청주·전주·천안 주차 조례는 '주차장 조례'(대전·광주 패턴 동일).
    # ⚠ 전주시는 2024-01 전북특별자치도 출범 → 지자체기관명 "전북특별자치도 전주시".
    ("경기도 성남시", "성남시 도시계획 조례"),
    ("경기도 성남시", "성남시 건축 조례"),
    ...
    ("충청남도 천안시", "천안시 건축 조례"),
    ("충청남도 천안시", "천안시 주차장 조례"),
    # ── 친환경(완화·혜택 Phase 3 보강): 녹색건축물 조성 지원 조례 17개(13+4) ──
    # 완화율은 국가 고시 별표9로 전국 동일 — 조례는 재정지원(그린리모델링 기금·노후주택 보조) 차이 surface용.
...
PREC_CAP = 100   # 대법원 판례 상한
EXPC_CAP = 150   # 법령해석례 상한
